...
_HOP_TYPE_SPLIT_RE = re.compile(r":|->")


def _simplify_hop(label):
    """'RTR_X[Router:F=REF(Y)]' -> 'RTR_X[Router]'
       'LKP_A[Unconnected Lookup->table:tbl]' -> 'LKP_A[Unconnected Lookup]'
       'RTR_X[Router]' -> 'RTR_X[Router]' (already simple, unchanged)
    Keeps only the transformation name and its bare type inside brackets,
    dropping any port/expression/lookup detail that follows a ':' or '->'."""
    m = _HOP_LABEL_RE.match(label)
    if not m:
        return label
    name, inner = m.group(1), m.group(2)
    ttype = _HOP_TYPE_SPLIT_RE.split(inner, maxsplit=1)[0].strip()
    return f"{name}[{ttype}]" if ttype else name
...
def format_lineage_chain(path):
    ...
    tracing backward) into the display string, source -> target order:
        ...
        'SQ_X[Source Qualifier] -> EXP_X[Expression] -> RTR_X[Router]'

    Each hop is reduced to 'Name[Type]' only, and consecutive duplicate
    hops (same simplified name+type back to back) are collapsed."""
    if not path:
        return ""
    simplified = [_simplify_hop(h) for h in reversed(path)]
    deduped = []
    for hop in simplified:
...
def build_transformation_catalog(folder_idx):
    """Folder-wide catalog of every transformation INSTANCE (across every
    mapping), deduplicated by instance name. One row per unique
    transformation instance name:
        Transformation Name, Transformation Type, Business Logic, Ports
    Business Logic is left blank for now. Ports is the sorted list of
    port/field names exposed by that transformation's definition."""
    catalog = {}  # name -> row dict, first occurrence wins (dedup)
    ...
    for mapping_name, mapping_node in folder_idx["mappings"].items():
        ...
        for inst_name, inst in m["instances"].items():
            ...
            ports = sorted(tdef["fields"].keys()) if tdef else []
    ...
    return sorted(catalog.values(), key=lambda r: r["Transformation Name"])
