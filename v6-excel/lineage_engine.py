    ...
    for scope_name, scope_node in scope_items:
        ...
        for inst_name, inst in m["instances"].items():
            ...
            for port_name in known_fields:
                ...
                # Business Logic / Additional Informations / Category are
                # intentionally left blank for both Tab-2 (Transformations)
                # and Tab-4 (Mapplet Transformations) catalogs - both share
                # this row-building function, so blanking here covers both.
                # The XML-derived text IS still computed here, though, and
                # stashed in hidden bookkeeping columns - business_logic_
                # enricher.py uses it as the "gap fill" fallback when the
                # user-supplied Excel workbook has nothing for this exact
                # (session/mapping/mapplet, transformation, port), per the
                # "Excel first, XML gap-fill" rule.
                generic_business_logic = "\n".join(logic_lines)
                xml_business_logic, xml_additional_info = _business_logic_and_additional_info(
                ...
                row = {
                    ...
                    "_Port": port_name,
                    # Hidden bookkeeping only (never a visible column, never
                    # part of CATALOG_DISPLAY_COLS/MAPPLET_TRANSFORM_CATALOG_
                    # DISPLAY_COLS): whether this port's OWN field carries an
                    # expression, i.e. whether logic is actually implemented
                    # on it vs. being a bare pass-through. Consumed by
                    # business_logic_enricher.py to compute the "Category"
                    # column ("Involves Derivation" / "Direct Pass through")
                    # the same way regardless of transformation type.
                    "_Own_Expression": own_expr,
                    # Hidden bookkeeping only: the XML-derived Business
                    # Logic / Additional Informations for this row, used as
                    # the gap-fill fallback by business_logic_enricher.py.
                    "_XML_Business_Logic": xml_business_logic,
                    "_XML_Additional_Info": xml_additional_info,
                }
...
# --------------------------------------------------------------------------
#
# Scans the ALREADY-BUILT-AND-ENRICHED Tab-2 (build_transformation_catalog)
# and Tab-4 (build_mapplet_transformation_catalog) row lists - i.e. this
# must be called AFTER business_logic_enricher.enrich_catalog_rows() has
# populated "Business Logic" (Excel first, XML gap-fill) - and pulls out
# just the rows that look like eligibility/qualification logic, per the
# "Where to look" spec:
...
def _is_eligibility_row(ttype, port_name, business_logic):
    if not business_logic or not str(business_logic).strip():
        return False
    t = (ttype or "").upper()
...
def build_eligibility_catalog(catalog_rows, mapplet_transform_rows, folder_idx, session_to_mapping):
    """Folder-wide eligibility-rules catalog, per the 7-column spec.

    catalog_rows / mapplet_transform_rows: the row lists returned by
    build_transformation_catalog() / build_mapplet_transformation_catalog()
    AFTER business_logic_enricher.enrich_catalog_rows() has run on each -
    i.e. "Business Logic" already reflects Excel-first/XML-gap-fill, and
    "_Logic_Source" already records which one actually supplied it.
    session_to_mapping: {session_name: mapping_name} - the FULL map
    (every session across the workflow(s) in scope), used only to map a
    mapping back to the session(s) that run it.
    """
    ...
    for r in catalog_rows:
        biz = r.get("Business Logic", "")
        if not _is_eligibility_row(r["Transformation Type"], r["PORT_NAME"], biz):
            continue
        ...
        rows.append({
            ...
            "Eligibility Rule/Logic (Technical)": biz,
            "Eligibility Rule/Logic (Plain Language)": plain_language_translate(biz),
            "Source (Excel/XML)": r.get("_Logic_Source") or "XML - lineage",
        })
    ...
    for r in mapplet_transform_rows:
        biz = r.get("Business Logic", "")
        if not _is_eligibility_row(r["Transformation Type"], r["PORT_NAME"], biz):
            continue
        ...
        for calling_mapping in callers:
            if calling_mapping is not None:
                label = f"{calling_mapping} -> {mapplet_name}"
                sess = sessions_for(calling_mapping)
            else:
                label = f"(unresolved) -> {mapplet_name}"
                sess = "(no session found)"
            rows.append({
                "Session": sess,
    ...
    # -- Collapse "whole-transformation" duplicates. Filter/Lookup/Update
    #    Strategy/Source Qualifier conditions are TABLE-level attributes in
    #    PowerCenter (they apply to the whole transformation, not to any one
    #    port) - build_transformation_catalog still emits one catalog row
    #    per port, though, so if only SOME of a transformation's ports had
    #    an Excel match and others fell back to XML gap-fill, the identical
    #    rule can surface twice (once per Source). Router is intentionally
    #    excluded here - its condition genuinely varies per group/port, so
    #    those rows must stay separate.
    WHOLE_TRANSFORM_TTYPES = ("FILTER", "LOOKUP", "UPDATE STRATEGY", "SOURCE QUALIFIER")

    def _is_whole_transform_type(ttype):
        t = (ttype or "").upper()
        return any(w in t for w in WHOLE_TRANSFORM_TTYPES)

    collapsed = {}
    collapse_order = []
    passthrough = []
    for r in rows:
        if not _is_whole_transform_type(r["Transformation Type"]):
            passthrough.append(r)
            continue
        key = (r["Session"], r["Mapping/Mapplet"], r["Transformation Name"], r["Transformation Type"])
        if key not in collapsed:
            collapsed[key] = r
            collapse_order.append(key)
        else:
            existing = collapsed[key]
            existing_is_excel = str(existing.get("Source (Excel/XML)", "")).startswith("Excel")
            new_is_excel = str(r.get("Source (Excel/XML)", "")).startswith("Excel")
            if new_is_excel and not existing_is_excel:
                collapsed[key] = r
    rows = passthrough + [collapsed[k] for k in collapse_order]
