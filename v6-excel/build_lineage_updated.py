...
from eligibility_extractor import (
    extract_eligibility_rows, append_eligibility_tab_to_html,
    ELIGIBILITY_DETAIL_COLS, ELIGIBILITY_SUMMARY_COLS,
)
...
def find_target_instance(folder_idx, target_table_name, target_instance_name, target_mapping_name):
    """Locates the single TARGET-type INSTANCE identified unambiguously by
    the caller-supplied (TARGET_TABLE, TARGET_INSTANCE, Target_Mapping)
    triple:
        target_table_name    -> the Table Name written to
        target_instance_name -> the TARGET instance name (mostly the
                                 sink/last transformation) that writes it
        target_mapping_name  -> the Mapping the target table is present in

    Returns the matching target definition name (TRANSFORMATION_NAME), or
    None if the mapping doesn't contain a TARGET instance with that name
    writing that table. Raises SystemExit if target_mapping_name itself
    isn't a known mapping, since that's an unambiguous input error worth
    surfacing distinctly from "not found"."""
    mnode = folder_idx["mappings"].get(target_mapping_name)
    if mnode is None:
        raise SystemExit(f"Mapping '{target_mapping_name}' not found in repository.")

    for ch in mnode["children"]:
        if ch["tag"] == "INSTANCE":
            a = ch["attributes"]
            if a.get("TYPE") == "TARGET" and a.get("NAME") == target_instance_name:
                tdefname = a.get("TRANSFORMATION_NAME")
                tdef = folder_idx["targets"].get(tdefname)
                if tdef and tdef.get("table") == target_table_name:
                    return tdefname
    return None


def build_lineage(json_path, target_table_name, target_instance_name, target_mapping_name,
                   workflow_name=None, out_prefix="lineage",
                   out_dir=None, business_logic_excel=None):
    ...
    # Reverse of session_to_mapping (mapping -> the session that runs it),
    # used only when a --business-logic-excel source is supplied, to
    # resolve the "Session Name" match key for Tab-2 rows. A mapping run by
    # more than one session keeps the first one encountered.
    mapping_to_session = {}
    for s, mp in session_to_mapping.items():
        mapping_to_session.setdefault(mp, s)

    business_logic_workbook = None
    if business_logic_excel:
        business_logic_workbook = load_source_workbook(business_logic_excel)

    # NEW (additive): "Eligibility Rules" / "Eligibility Rules - Summary"
    # tabs. Extracted strictly from business_logic_workbook (the same
    # --business-logic-excel workbook, per requirement), with folder_idx
    # used only to resolve mapplet-to-mapping usage and as an alias
    # gap-fill - see eligibility_extractor.py. A no-op (both tabs empty)
    # when no --business-logic-excel was supplied; never affects any other
    # output, and any failure here is caught so it can't break the rest of
    # the report.
    try:
        df_eligibility_detail, df_eligibility_summary = extract_eligibility_rows(
            business_logic_workbook, folder_idx, session_to_mapping, mapping_to_session
        )
    except Exception as e:
        print("Eligibility Rules extraction skipped:", e)
        df_eligibility_detail = pd.DataFrame(columns=ELIGIBILITY_DETAIL_COLS)
        df_eligibility_summary = pd.DataFrame(columns=ELIGIBILITY_SUMMARY_COLS)

    # locate target instance - fully disambiguated by the caller-supplied
    # (TARGET_TABLE, TARGET_INSTANCE, Target_Mapping) triple, so there's no
    # more "guess the earliest session" heuristic needed: the mapping is
    # given directly.
    anchor_mapping = target_mapping_name
    anchor_tdefname = find_target_instance(
        folder_idx, target_table_name, target_instance_name, target_mapping_name)
    if not anchor_tdefname:
        raise SystemExit(
            f"No TARGET instance named '{target_instance_name}' writing table "
            f"'{target_table_name}' was found in mapping '{target_mapping_name}'."
        )

    sessions_using = [s for s, mp in session_to_mapping.items() if mp == anchor_mapping]
    if not sessions_using:
        raise SystemExit(f"Mapping '{anchor_mapping}' is not run by any session in scope.")

    candidate_orders = sorted((session_order[s], s) for s in sessions_using)
    anchor_order, anchor_session = candidate_orders[0]

    print(f"Target table '{target_table_name}' / instance '{target_instance_name}' -> "
          f"mapping '{anchor_mapping}', session '{anchor_session}' "
          f"(execution order {anchor_order})")
    if len(candidate_orders) > 1:
        print("  Note: multiple sessions run this mapping:")
    ...
    catalog_rows = build_transformation_catalog(folder_idx)
    # Business Logic / Additional Informations / Category enrichment: when a
    # --business-logic-excel source was supplied, values are looked up from
    # it per-type (see business_logic_enricher.py); Category is always
    # (re)computed regardless, from the _Own_Expression bookkeeping field.
    # No-op (existing behaviour preserved) when business_logic_workbook is None.
    enrich_catalog_rows(catalog_rows, business_logic_workbook, mapping_key="_Mapping",
                         mapplet_mode=False, session_lookup=mapping_to_session)
    df_catalog_full = pd.DataFrame(catalog_rows)
    ...
    # Same enrichment as Tab-2, scoped to mapplets (mapplet_mode=True skips
    # the Session Name match key, since a mapplet definition isn't itself
    # run by any one session - see business_logic_enricher.py).
    enrich_catalog_rows(mapplet_transform_rows, business_logic_workbook, mapping_key="_Mapplet",
                         mapplet_mode=True, session_lookup=None)
    ...
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            ...
            # NEW (additive): Eligibility Rules tabs, written last, and
            # only when at least one eligibility rule was found anywhere
            # (spec: "If no eligibility logic is found anywhere in the
            # repo, both tabs are omitted entirely from every output
            # format (no empty tab)").
            if len(df_eligibility_detail) > 0:
                df_eligibility_detail.to_excel(writer, sheet_name="Eligibility Rules", index=False)
    ...
    # NEW (additive): append the Eligibility Rules tab to the HTML report
    # already written above, as the LAST tab. No-op / non-destructive if
    # the report wasn't written or the injection fails for any reason.
    if html_written:
        append_eligibility_tab_to_html(html_path, df_eligibility_detail, df_eligibility_summary)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Usage: python build_lineage.py repo.json TARGET_TABLE TARGET_INSTANCE "
                     "Target_Mapping [--business-logic-excel business_logic_source.xlsx]"
    )
    ...
    ap.add_argument("--business-logic-excel", default=None,
                     help="Optional path to a user-supplied workbook (one tab per "
                          "transformation type, plus its Mapplet_<Type> counterpart - "
                          "Expression, Filter, Joiner, Lookup Procedure, Router, Sequence, "
                          "Sorter, Source Qualifier, Stored Procedure, Transaction Control, "
                          "Update Strategy, Aggregator, Rank, Custom Transformation) used to "
                          "fill in Business Logic / Additional Informations / Category on the "
                          "Transformations and Mapplet_Transformations tabs. See "
